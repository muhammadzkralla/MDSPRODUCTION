...
import com.dimits.mahalladeliveryserver.callback.MyButtonClickListener;
...
public abstract class MySwiperHelper extends ItemTouchHelper.SimpleCallback {
    ...
    private synchronized void recoverSwiperItem() {
    ...
    public class MyButton{
        ...
        public void onDraw(Canvas c,RectF rectF,int pos) {
            ...
            float cWight = rectF.width();
            p.setTextAlign(Paint.Align.LEFT);
            p.getTextBounds(text,0,text.length(),r);
            float x=0,y=0;
            if (imageResId == 0) {   // show text

                x = cWight/2f - r.width()/2f - r.left;
                y = cHeight/2f + r.height()/2f - r.bottom;
                c.drawText(text,rectF.left+x,rectF.top+y,p);
            }
            else {              // show image resorce
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if(dX < 0) {
                ...
                translation = dX*buffer.size()*buttonWidth/itemView.getWidth();
                drawButton(c,itemView,buffer,pos,translation);
            }
        }
        super.onChildDraw(c,recyclerView,viewHolder,translation,dY,actionState,isCurrentlyActive);
    }

    private void drawButton(Canvas c, View itemView, List<MyButton> buffer, int pos, float translation) {
        float right = itemView.getRight();
        float dButtonWidth =-1*translation / buffer.size();
